...
class CNN(nn.Module):
    ...
    def __init__(self, dropout_prob):
        ...
        self.conv1 = nn.Conv2d(in_channels = 1,out_channels=8, kernel_size=(5, 5), stride=1, padding="same")
        self.relu1 = nn.ReLU()
        self.maxpool1 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)

        self.conv2 = nn.Conv2d(in_channels = 8, out_channels=16, kernel_size=(3, 3), stride=1, padding="valid")
        self.relu2 = nn.ReLU()
        self.maxpool2 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)

        self.aff1 = nn.Linear(in_features=576, out_features = 600)
        self.relu3 = nn.ReLU()
        self.drop = nn.Dropout2d(p=0.3)

        self.aff2 = nn.Linear(in_features=600, out_features = 120)
        self.relu4 = nn.ReLU()

        self.aff3 = nn.Linear(in_features=120, out_features=10)
    ...
    def forward(self, x):
        ...
        output = self.aff1(output)
        output = self.relu3(output)
        output = self.drop(output)

        output = self.aff2(output)
        output = self.relu4(output)

        output = self.aff3(output)
...
def plot_feature_maps(model, train_dataset):
    ...
    plt.savefig('original_image_0005.png')
    ...
    for i in range(act.size(0)//3):
        for j in range(act.size(0)//2):
            ...
            k+=1  
            plt.savefig('activation_maps_0005.png') 
...
0.0005
Valid acc: 0.9843
Final Test acc: 0.9598
0.00001
Final Test acc: 0.8896
'''