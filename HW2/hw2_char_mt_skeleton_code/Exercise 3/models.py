...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        return torch.transpose(out, 0, 1), hidden
...
class Decoder(nn.Module):
    ...
    def _forward_pass(self, tgt, 
                      dec_state, 
                      encoder_outputs, 
                      src_lengths=None):
        # pass tgt through embedding layer
        tgt_embedded = self.embedding(tgt)
        
        # apply dropout on the embedded tgt
        tgt_embedded = self.dropout(tgt_embedded)

        # pass the embedded tgt and the initial decoder state through the LSTM
        dec_output, dec_state = self.lstm(tgt_embedded, dec_state)

        # compute attention weights
        # attn_weights = self.attn(dec_output, encoder_outputs, src_lengths)

        # apply attention to the encoder outputs
        # context = attn_weights.bmm(encoder_outputs)

        # concatenate the context vector and the decoder output
        # dec_output = torch.cat((context, dec_output), dim=-1)

        # pass the concatenated vector through a linear layer to produce the final output
        output = self.fc(dec_output)

        return output, dec_state

        # for emb_t in embs.split(1, dim=1):
        #     dec_in = torch.cat([emb_t.squeeze(1), input_feed])
    ...
    def forward(
        ...
        #     )
        #############################################
        # create an empty list to store the outputs for each input in the batch
        dec_outs = []   

        # iterate over the batch
        for i in range(tgt.size(0)):
            # select the i-th input and its corresponding state, encoder outputs, and src_lengths
            tgt_i = tgt[i, :]
            dec_state_i = (dec_state[0][:, i, :], dec_state[1][:, i, :])
            enc_out_i = encoder_outputs[i, :, :]
            src_len_i = src_lengths[i]
            
            # pass the i-th input through the decoder
            dec_out_i, dec_state_i = self._forward_pass(tgt_i, dec_state_i, enc_out_i, src_len_i)
            
            # append the output for the i-th input to the dec_outs list
            dec_outs.append(dec_out_i)

        # stack the outputs for each input in the batch along the first dimension
        dec_outs = torch.stack(dec_outs, dim=0)

        return dec_outs, dec_state_i
        #############################################
        # END OF YOUR CODE
        #############################################
